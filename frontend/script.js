...
submitBtn.addEventListener('click', async (e) => {
    ...
    try {
        ...
        if (data.success) {
            ...
            addBotMessage(`❌ Sorry, I encountered an error: ${data.message || 'Please try again later.'}`);
        }
...
function getFormData() {
    return {
        ...
        familyIncome: document.getElementById('familyIncome').value ? parseInt(document.getElementById('familyIncome').value) : null,
...
function addUserMessage(formData) {
    ...
    if (formData.educationLevel !== '12th') messageText += ` at ${formData.educationLevel} level`;
    messageText += '.';
    
    if (formData.state) messageText += ` I prefer colleges in ${formData.state}.`;
    if (formData.familyIncome) messageText += ` My family's annual income is ₹${formData.familyIncome.toLocaleString()}.`;
        ...
        data.eligibleColleges.slice(0, 6).forEach(college => {
            ...
            resultsHTML += `
                <div class="result-card">
                    ...
                    ${college.ranking && college.ranking.nirf ? `<p><strong>NIRF Rank:</strong> ${college.ranking.nirf}</p>` : ''}
                    ${courses ? `<p><strong>Available Courses:</strong> ${courses}</p>` : ''}
                    <div class="tags">
                        <span class="tag">${college.type}</span>
                        ${college.courses.map(course => `<span class="tag">${course.stream || 'General'}</span>`).join('')}
                    </div>
                </div>
            `;
        });
        ...
        data.eligibleScholarships.slice(0, 6).forEach(scholarship => {
            resultsHTML += `
                <div class="result-card">
                    ...
                    <p><strong>Amount:</strong> ₹${scholarship.amount.value.toLocaleString()} (${scholarship.amount.type})</p>
