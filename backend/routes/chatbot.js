...
// Chatbot endpoint for student queries
  ...
  try {
    ...
    return colleges.filter(college => {
      ...
      const eligibleCourses = college.courses.filter(course => {
        ...
        const requiredPercentage = course.eligibilityPercentage?.[category.toLowerCase()];
...
function generateChatbotResponse({ percentage, stream, category, eligibleColleges, eligibleScholarships }) {
  ...
  if (eligibleColleges.length > 0) {
    ...
    eligibleColleges.slice(0, 5).forEach((college, index) => {
      response += `${index + 1}. ${college.name} - ${college.location.city}, ${college.location.state}\n`;
      response += `   Type: ${college.type}`;
      if (college.ranking?.nirf) response += ` | NIRF Rank: ${college.ranking.nirf}`;
      response += '\n';
    });
  ...
  if (eligibleScholarships.length > 0) {
    ...
    eligibleScholarships.slice(0, 5).forEach((scholarship, index) => {
      ...
      response += `   Provider: ${scholarship.provider} | Amount: ₹${scholarship.amount.value.toLocaleString()} (${scholarship.amount.type})\n`;
      response += `   Type: ${scholarship.type}\n`;
    });
    if (eligibleScholarships.length > 5) {
      response += `   ... and ${eligibleScholarships.length - 5} more scholarships\n`;
    }
    response += '\n';
  } else {
